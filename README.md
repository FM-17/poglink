...
- You'll also need to decide where you want to store this configuration file. By default, the bot will look for it in `~/.ark-discord-bot`, but you can change the location via the `-d` CLI argument. 
...
- For example, if you chose to use the default, you'd need to move the `config.yaml` file to `~/.ark-discord-bot/config.yaml`

#### Installing from PyPi

Run the following command to install from PyPi

    pip install ark-discord-bot
...
```
docker pull travipross/ark-discord-bot
```
